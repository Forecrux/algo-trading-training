...
class MyStrategy(strategy.BacktestingStrategy):
    ...
    def onBars(self, bars):   
        ...
        print "Price %s" % (bar.getPrice())
        print "Volume %s" % (bar.getVolume())
        print "Price Zigzag        %s" % (pivots)
        x = np.array(closeDs)
        print "Price Zigzag change %s" % compute_segment_returns(x, pivots)
            
        pivotV = peak_valley_pivots(volumeDs, 0.2, -0.2)
        y = np.array(volumeDs)
        print "Volume Zigzag change%s" % compute_segment_returns(y, pivots)
        print (" ")
